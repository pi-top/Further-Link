...
@pitop.route('/process/init', methods=['POST'])
def list_create():
...
def list_file(tree_vo, workspace, codes):
    ...
    for file in file_list:
        ...
        if os.path.isfile(tree_vo_tmp.file_path):
            for code in codes:
                if (code["projectVersionId"] + "/" + code["path"]).__eq__(tree_vo_tmp.file_path.replace(workspace, "")):
                    tree_vo_tmp.id = code["id"]

            tree_vo_tmp.is_leaf = True
            list.append(tree_vo_tmp.__dict__)
        elif os.path.isdir(tree_vo_tmp.file_path):
            list.append(list_file(tree_vo_tmp.__dict__, workspace, codes))

    tree_vo["children"] = list
    return tree_vo


@pitop.route('/process/rename', methods=['PUT'])
def rename():
    """
...
@pitop.route('/process/read', methods=['GET'])
def read_file():
    """
...
@pitop.route('/process/delete', methods=['DELETE'])
def delete():
    """
    remove file
    :return: True
...
@pitop.route('/process/create', methods=['POST'])
def create():
    """
...
@pitop.route('/process/exec', methods=['GET'])
def exe():
    """
    exec a python file
...
@pitop.route('/process/stop', methods=['GET'])
def stop():
    """
    stop exec thread
    :return:
