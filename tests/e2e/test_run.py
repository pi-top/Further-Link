...
async def test_bad_message(run_ws_client):
    # this message has no process id
...
"""
    ...
    commands = create_message("stdin", "1", {"input": code}, "1")
...
print(2)
"""
    start_cmd = create_message("start", "1", {"runner": "python3", "code": code1}, "1")
    await run_ws_client.send_str(start_cmd)

    await receive_data(run_ws_client, "started", process="1")

    start_cmd = create_message("start", "1", {"runner": "python3", "code": code2}, "2")
    await run_ws_client.send_str(start_cmd)
...
async def test_bad_code(run_ws_client):
    ...
    m_type, m_data, m_process, _ = parse_message(message.data)
    assert m_type == "stderr"
    lines = m_data["output"].split("\n")
    assert lines[0].startswith("  File")
    assert lines[1] == "    i'm not valid python"
    assert lines[2][-1] == "^"
    assert lines[3] in ("SyntaxError: EOL while scanning string literal", "SyntaxError: unterminated string literal (detected at line 1)")
