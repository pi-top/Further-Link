...
while "BYE" != s:
    ...
    await wait_for_data(run_ws_client, 'stdout', 'output',
                        'HUH?! SPEAK UP, SONNY!\n', 0, '1')
...
async def test_two_clients(run_ws_client):
    async with aiohttp.ClientSession() as session2:
        async with session2.ws_connect(RUN_URL) as run_ws_client2:
            ...
            await wait_for_data(run_ws_client, 'stopped', 'exitCode', -15,
                                100, '1')

            stop_cmd = create_message('stop', None, '1')
            await run_ws_client2.send_str(stop_cmd)

            await wait_for_data(run_ws_client2, 'stopped', 'exitCode', -15,
                                100, '1')
...
async def test_discard_old_input(run_ws_client):
    ...
    unterminated_input = create_message(
        'stdin', {'input': 'unterminated input'}, '1')
